...
from examples import gmpy2_pkcs10aep_cipher
...
def generate_key_pair(user_id):
    ...
    filepath = "privkeys/{user_id}".format(user_id=user_id) + "/private.pem"
...
def get_decrypt_decoding(user_id):
    filepath = "privkeys/{user_id}".format(user_id=user_id) + "/private.pem"
    ...
    if not private_key:
        try:
            ...
            raise PrivkeyNotFoundError("Private key not found, user_id: {user_id}".format(user_id=user_id))

        # redis_client.setex(cache_key, 120, private_key)
        redis_client[cache_key] = private_key
...
def decrypt(encrypted_text, user_id):
    rsa_key, cipher_rsa = get_decrypt_decoding(user_id)
